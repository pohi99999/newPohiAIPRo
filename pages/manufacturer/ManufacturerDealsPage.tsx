// pages/manufacturer/ManufacturerDealsPage.tsx
...
const ManufacturerDealsPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const myDeals = allMatches
                .filter(deal => deal.stockDetails.uploadedByCompanyId === userCompany.id || (userRole === UserRole.MANUFACTURER && !deal.stockDetails.uploadedByCompanyId))
             ...
             {deals.length === 0 ? (
                <Card>
                    <div className="text-center py-12">
                        ...
                        <Button to="/manufacturer/matches" variant="primary" className="mt-4">{t('menu_my_matches')}</Button>
                    </div>
                </Card>
            ) : (
                        ...
                        <Card key={deal.id} className="hover-glow transition-all duration-300">
                           <div className="p-4">
                                ...
                                <div className="flex flex-col md:flex-row gap-4 items-stretch">
                                    <ItemDetails item={deal.stockDetails} title={t('deals_yourStock')} />
                                    ...
                                    <div className="flex-1">
                                        <h4 className="text-md font-semibold text-cyan-300 mb-2">{t('deals_partner')} ({t('userRole_CUSTOMER')})</h4>
                                        <div className="p-3 rounded-lg bg-slate-700/50 text-xs space-y-1">
                                            <p className="font-semibold text-sm text-slate-200">{deal.demandDetails.submittedByCompanyName}</p>
                                            <p className="text-slate-300">
                                                <span className="font-bold">{deal.demandDetails.productName}</span>: {t('deals_status')}: <span className="font-semibold">{getTranslatedDemandStatus(deal.demandDetails.status, t)}</span>
                                            </p>
...
export default ManufacturerDealsPage;